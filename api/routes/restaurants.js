...
// GET /restaurants/filter → Filter restaurants by borough and/or minimum score
router.get('/filter', async (req, res) => {
  try {
    console.log('Filter request received with params:', req.query);
    const { city, score } = req.query;
    const query = {};
    if (city) query.borough = { $regex: city, $options: 'i' };
    if (score) query['grades.0.score'] = { $gte: parseFloat(score) };
    if (!city && !score) {
      return res.status(400).json({ error: 'At least one filter (city or score) is required' });
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (street) updateFields['location.street'] = street;
    if (building) updateFields['location.building'] = building;
    if (zipcode) updateFields['location.zipcode'] = zipcode;
    if (coord_longitude || coord_latitude) {
      updateFields['location.coordinates'] = [
        parseFloat(coord_longitude) || 0,
        parseFloat(coord_latitude) || 0
      ];
    }
